...
import useGetAllCampaignContents from '@/hooks/useGetAllCampaignContents';
...
function CampaignsContentsPage() {
  const {
    data: contents = [],
    isLoading,
    error,
    refetch: refreshContents,
  } = useGetAllCampaignContents();
  ...
  // Extract unique campaign options for the filter
  const campaignOptions = [
    { value: 'All Campaigns', label: 'All Campaigns' },
    ...Array.from(
      new Set(
        contents
          .filter((content: any) => content.application?.campaign)
          .map((content: any) => content.application.campaign._id)
      )
    ).map((campaignId) => {
      const campaignName =
        contents.find(
          (content: any) => content.application?.campaign?._id === campaignId
          //@ts-ignore
        )?.application?.campaign?.name || 'Unknown Campaign';
  ...
  // Filter contents based on active tab, search term, platform filter, and campaign filter
  const filteredContents = contents.filter((content: any) => {
    ...
    if (
      ...
      !content.application?.campaign?.name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase())
    ) {
      return false;
    ...
    // Filter by campaign
    if (campaignFilter.value !== 'All Campaigns') {
      if (content.application?.campaign?._id !== campaignFilter.value) {
        return false;
      }
    }
  ...
  const handleUpdateContentStatus = async (
    ...
    try {
      ...
      toast.error(`Failed to ${status} content. Please try again.`);
    } finally {
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.application?.campaign, {
      id: 'campaign',
      header: 'CAMPAIGN',
      cell: (info) => {
        ...
        return campaign ? (
          ...
          >
            <div className='w-10 h-10 relative rounded overflow-hidden mr-4'>
              {' '}
              <Image
                src={
                  campaign.image
          ...
          <span className='text-gray-500'>Unknown Campaign</span>
        );
      },
    }),
  ...
  return (
    <div className='md:p-4 bg-gray-100 dark:bg-gray-900 min-h-screen'>
      ...
      >
        <div className='p-4'>
          {selectedContent && (
            <div>
              <div className='mb-4'>
                <h3 className='text-md font-semibold mb-2'>Campaign</h3>
                <Link
                  href={`/campaigns/${selectedContent.application?.campaign?._id}`}
                  className='text-blue-500 hover:underline'
                >
                  {selectedContent.application?.campaign?.name ||
                    'Unknown Campaign'}
                </Link>
              </div>
              ...
              <div className='mb-4'>
                ...
                <Link
                  href={`/users/${selectedContent.application?.influencer?._id}`}
                  className='text-blue-500 hover:underline'
                >
      ...
      >
        <div>
          <p className='mb-4'>
            The content has been rejected. Please provide a reason for the
            rejection.
          </p>
          <TextArea
...
export default CampaignsContentsPage;
